...
import json
import os
# import useful functions for primes related actions we made
...
def handle_rsa(conn, data, alice_message):
    response = json.loads(data.decode())
    public_key_e = response["public"]
    public_key_n = response["parameter"]["n"]

    aes_key = os.urandom(32)
    encrypted_key = [pow(byte, public_key_e, public_key_n)
                     for byte in aes_key]
    ...
    if encrypted_data:
        response = json.loads(encrypted_data.decode())
        logging.debug(f"{response}")
        if response.get("opcode") == 2:
            encrypted_message = response.get("encryption")
            logging.info("Received encrypted message from Bob")

            decrypted_message = ec.decrypt(
                aes_key, encrypted_message).decode().strip()
        ...
        logging.error("No data received from Bob for RSA encrypted message")
        error_message = {
            "opcode": 3,
...
def handle_dh_exchange(conn, data, alice_message):
    ...
    if encrypted_data:
        ...
        logging.error(
            "No encrypted message received from Bob for AES exchange.")
        error_message = {
            "opcode": 3,
            "error": "No encrypted message received from Bob for AES exchange."
        }
...
def run(args):
    ...
    if args.type == "RSAKey":
        ...
        data = conn.recv(4096)
        if data:
            logging.info("data received from Bob for RSA key validation")
            handle_rsa_key(data)
        else:
            logging.error("No data received from Bob for RSA key validation")
    elif args.type == "RSA":
        send_rsa_encryption_request(conn)

        data = conn.recv(4096)
        if data:
            logging.info("data received from Bob for RSA")
            handle_rsa(conn, data, args.message)
        else:
            logging.error("No data received from Bob for RSA")
    elif args.type == "DH":
...
def command_line_args():
    ...
    parser.add_argument("-a", "--addr", metavar="<alice's address>",
                        help="Alice's address", type=str, required=True, default="127.0.0.1")
    parser.add_argument("-p", "--port", metavar="<alice's port>",
                        help="Alice's port", type=int, required=True)
    parser.add_argument("-m", "--message", metavar="<alice's message>",
                        help="Alice would send this message if needed", type=str, required=True)
    ...
    parser.add_argument("--type", metavar="<type>",
                        help="Type of request (RSAKey, RSA, DH)", type=str)
